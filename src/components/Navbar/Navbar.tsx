...
const NavbarList = ({ children, className }) => {
  return (
    <ul
      className={`flex-grow  border-t border-indigo-400/20 hover:border-indigo-400/40 ${className}`}
    >
      {children}
    </ul>
  );
...
const NavbarListItem = ({ children }) => {
  return (
    <li className="my-2 rounded-lg bg-transparent hover:bg-indigo-400/40 cursor-pointer flex gap-2 items-center text-slate-300 hover:text-slate-100">
      {children}
    </li>
  );
};
